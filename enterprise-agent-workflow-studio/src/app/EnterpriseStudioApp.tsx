...
import { ArrowRight, CheckCircle2, ExternalLink, GitBranch, ShieldAlert } from "lucide-react";
  ...
  return (
    <main className="shell">
      ...
      <section className="grid two" style={{ marginTop: 24 }}>
        ...
        <div className="panel" style={{ padding: 24 }}>
          ...
          <div className="grid two">
            <div className="metric"><span className="muted">Readiness</span><strong>{workflow.readiness.status}</strong></div>
            <div className="metric"><span className="muted">Risk score</span><strong>{workflow.riskScore}/100</strong></div>
          </div>
      ...
      <section className="grid two" style={{ marginTop: 16 }}>
        <div className="panel" style={{ padding: 24 }}>
          ...
          <div className="grid two" style={{ marginTop: 12 }}>
            <label>Risk level<select className="field" value={riskLevel} onChange={(event) => setRiskLevel(event.target.value as WorkflowRequest["riskLevel"])}><option value="low">low</option><option value="medium">medium</option><option value="high">high</option></select></label>
            <label>Data class<select className="field" value={dataClass} onChange={(event) => setDataClass(event.target.value as WorkflowRequest["dataClass"])}><option value="public">public</option><option value="internal">internal</option><option value="regulated">regulated</option></select></label>
          </div>
        </div>
        <div className="panel" style={{ padding: 24 }}>
          <h2>Tool Registry</h2>
      ...
      <section className="panel" style={{ padding: 24, marginTop: 16 }}>
        <h2><GitBranch size={20} /> Generated Agent Graph</h2>
      ...
      <section className="grid two" style={{ marginTop: 16 }}>
        <div className="panel" style={{ padding: 24 }}>
          <h2>Approval Gates + Launch Checks</h2>
